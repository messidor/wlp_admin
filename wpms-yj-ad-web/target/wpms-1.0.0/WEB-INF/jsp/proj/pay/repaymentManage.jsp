...
<script type="text/javascript">
	...
	var columnDefs = [
		...
		*/
		...
		setColumn(["regDt", "요청일시", "center", 105]),
		setColumn(["payDt", "결제일시", "center", 105]),
		setColumn(["carNo", "차량번호", "center", 80]),
		setColumn(["memberName", "이름", "center", 75]),
		setColumn(["memberPhone", "연락처", "center", 80]),
		setColumn(["pGovName", "관리기관", "left", 160]),
		setColumn(["pGovAccountName", "통장명", "left", 170]), 
		setColumn(["pGovAccountNumber", "통장번호", "left", 100]),
		setColumn(["parkingName", "주차장명", "left", 100]),
		setColumn(["paymentGubnName", "결제구분", "center", 70]),
		setColumn(["parkingPrice", "주차요금", "right", 70]),
		setColumn(["confirmName", "처리자", "center", 80]),
		setColumn(["repayStatusName", "재결제요청상태", "center", 100], [grdPayGubn, ["button", null, "link", "text", ""] ]),
	];
	...
	function grdPayGubn(e, data) {
		if(data.repayStatus != 'D' && data.repayStatus == 'A') {
			if(data.paymentGubn == 'A'){
				OpenPopupSingle("/pay/repaymentManagePopup.do?chargeNo="+data.chargeNo+"&payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 960, "_Pop10");	
			}else{
				if(data.chargeNo.split(",").length > 1) {
					OpenPopupSingle("/pay/outstandingManagePopup.do?payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 790, "_Pop13");	
				}else{
					OpenPopupSingle("/pay/repaymentManagePopup.do?chargeNo="+data.chargeNo+"&payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 980, "_Pop16");
				}
			}
			
		}else if(data.repayStatus != 'D' && data.repayStatus == 'C'){
			if(data.paymentGubn == 'A'){
				OpenPopupSingle("/pay/repaymentManagePopup.do?chargeNo="+data.chargeNo+"&payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 925, "_Pop11");
			}else{
				if(data.chargeNo.split(",").length > 1) {
					OpenPopupSingle("/pay/outstandingManagePopup.do?payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 790, "_Pop15");	
				}else{
					//거절, 미납결제 한건만 있을때
					OpenPopupSingle("/pay/repaymentManagePopup.do?chargeNo="+data.chargeNo+"&payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 925, "_Pop17");
				}
			}
		}else if(data.repayStatus != 'D' && data.repayStatus == 'B'){
			if(data.paymentGubn == 'A'){
				OpenPopupSingle("/pay/repaymentManagePopup.do?chargeNo="+data.chargeNo+"&payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 840, "_Pop12");
			}else{
				if(data.chargeNo.split(",").length > 1){
					OpenPopupSingle("/pay/outstandingManagePopup.do?payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 790, "_Pop14");
				}else{
					OpenPopupSingle("/pay/repaymentManagePopup.do?chargeNo="+data.chargeNo+"&payCode="+data.payCode + "&paySeq=" + data.paySeq + "&memberId=" + data.memberId + "&applyCode=" + data.applyCode + "&repayStatus=" + data.repayStatus, 1100, 840, "_Pop18");
				}
			}
		}
	}
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "pay.repaymentManage.Select_List");
	...
	function downExcelData() {
		...
		frm.addInput("query_id", "pay.repaymentManage.Select_List");
		frm.addInput("type", "repaymentManage");
			...
			<div class="search-area col-lg-12">
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						...
						<form:select id="kStatus" caption="재결제요청상태" all="true" allLabel="전체" queryId="#REPAY_STATUS" value="A"/>
					</div>
				</form>
			</div>
			<card:close />
