...
<script type="text/javascript">
	...
	var columnDefs = [
		...
		*/
		...
		setColumn(["publishCouponCode", "할인권 발급 코드"], [], true),
		setColumn(["pGovCode", "주차장 기관 코드"], [], true),
		setColumn(["useDt", "적용일시", "center", 100]),
		setColumn(["pGovName", "관리기관", "left", 150]),
		setColumn(["parkingName", "주차장명", "left", 150]),
		setColumn(["carNo", "차량번호", "center", 70]),
		setColumn(["memberName", "회원명", "center", 60]),
		setColumn(["couponName", "할인권명", "left", 150]),
		setColumn(["useQty", "사용수량", "right", 50]),
		setColumn(["discount", "할인 금액/시간", "right", 80]),
		setColumn(["useYn", "사용여부", "center", 50]),
	];

	// setColumnMask(컬럼옵션변수, 컬럼명, 타입, 추가옵션)
	setColumnMask(columnDefs, "useDt", "datetime");
	setColumnMask(columnDefs, "useQty", "number");
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "pay.couponUse.Select_List");
	...
	function downExcelData() {
		...
		frm.addInput("query_id", "pay.couponUse.Select_List");
		frm.addInput("type", "couponUse");
	...
	$(function() {
		...
		$("#kSearchDate, #kParkingNo, #kCarNo, #kMemberName, #kUseYn").change(function() {
			loadTableData();
		});
...
<%-- 				<form:button type="Excel" id="btnExcel" /> --%>
			...
			<div class="search-area col-lg-12">
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						<form:input id="kSearchDate" caption="사용일자" addAttr="maxlength='100'" />
						<c:choose>  
						...
						<form:input id="kMemberName" caption="회원명" addAttr="maxlength='20'" />
						<form:select id="kUseYn" caption="사용구분" all="true" allLabel="전체" queryId="#USE_YN" />
					</div>			
				</form>
			</div>
			<card:close />
