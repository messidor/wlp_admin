...
<script type="text/javascript">
	 ...
	 */
			...
			setColumn(["outApiCode", "outApiCode", "center", 140], [], true),
			...
			setColumn(["paymentYn", "결제완료여부", "center", 100]),
			setColumn(["actualPayYn", "자동결제여부", "center", 100]),
			
			setColumn(["totalParkingFee", "총 주차금액(감면미적용)", "right", 160]),
			setColumn(["discountFee", "할인금액(감면제외)", "right", 130]),
			setColumn(["prePayFee", "사전정산금액(감면적용)", "right", 160]),
			setColumn(["parkingFee", "결제요청금액(감면적용)", "right", 160]),
			setColumn(["chargePrice", "결제금액(감면적용)", "right", 130]),
			
			setColumn(["reduceCode", "감면코드", "center", 100]),
			setColumn(["reductionName", "감면종류", "left", 150]),
			setColumn(["reductionRate", "감면비율", "center", 100]),
	];    
	...
	function loadTableData() {
		...
		frm.addParam("regDt", $("#regDt").val());
		frm.addParam("query_id", "park.apiNewOutStatus.select_List");
		...
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"
			...
			<div class="search-area col-lg-12">
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					...
					<form:select id="kParkingNo" caption="주차장 인증코드" all="true" allLabel="전체" queryId="park.apiNewOutStatus.select_parkInfo"/>
