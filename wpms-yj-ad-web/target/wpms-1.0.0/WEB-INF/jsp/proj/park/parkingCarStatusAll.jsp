...
<script type="text/javascript">
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "park.parkingCarStatusAll.select_parkStatusAllList");
	...
	function downExcelData() {
		...
		frm.addInput("query_id", "park.parkingCarStatusAll.select_parkStatusAllList");
		frm.addInput("type", "parkingCarStatusAll");
	...
	$(function() {
		//일자
		$("#kEntryDate").initDateOnly("days",-14,"days",0);
		...
		// 주차상태,조회조건,주차장,일자 변경시
		$("#kSearchGubn, #kMemberYn, #kParkingNo, #kEntryDate").change(function(){
			loadTableData();
		});
				...
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					...
					<c:choose>  
						<c:when test="${user_role_id eq 'MANAGE'}"> 
							...
							<form:select id="kParkingNo" caption="주차장" all="false" allLabel="전체" queryId="select.select_parkingInfo"/>
						</c:when> 
						<c:otherwise> 
							<form:select id="kParkingGovCode" caption="관리기관" all="false" allLabel="전체" queryId="select.select_parkingGovInfo"/>
							<form:select id="kParkingNo" caption="주차장" all="false" allLabel="전체" queryId="select.select_parkingInfo"/>
