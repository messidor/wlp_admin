...
<script type="text/javascript">
	 ...
	 */
			...
			setColumn(["parkingName", "주차장명", "left", 90]),
            setColumn(["parkingAreaCode", "주차장코드", "center", 70]),
			setColumn(["gugunName", "구역(시/군/구)", "center", 80]),
			setColumn(["address", "주소", "left", 280]),
			setColumn(["parkingTel", "전화번호", "center", 80]),
			setColumn(["pCompName", "관리회사", "left", 90]),
			setColumn(["pGovName", "관리기관", "left", 150]),
			setColumn(["parkingSpot", "총 주차면수", "right", 80]),
			setColumn(["parkingSpotNow", "현재 주차면수", "right", 100]),
			setColumn(["spotBtn", "주차면수", "center", 80], [grdParkingNo, ["button", null, "inverse", "변경", ""]]),
			setColumn(["useYnName", "사용여부", "center", 70]),
            setColumn(["modId", "수정자", "center", 50]),
	...
	_gridHelper.addRowDblClickEvent(function(event) {
   		OpenPopupSingle("/park/parkingManagePopup.do?parkingNo=" + event.data.parkingNo, 1950, 950, "_Pop6");
	});
	
	function grdParkingNo(e, data) {
		OpenPopupSingle("/park/parkingSpotPopup.do?parkingNo=" + data.parkingNo, 600, 310, "_Pop15");
	}
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "park.parkingManage.select_parkList");
	...
	$(function() {
		...
		$("#btnRegister").click(function() {
			OpenPopupSingle("/park/parkingManagePopup.do", 1950, 950, "_Pop");
		});
		...
		$("#kParkingNo,#kParkingCompCode,#kUseYn").change(function() {
			loadTableData();
		});
				...
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<c:choose>
						<c:when test="${fn:contains(user_role_id, 'MANAGE') || fn:contains(user_role_id, 'AD')}">
							...
							<form:select id="kParkingNo" caption="주차장" all="true" allLabel="전체" queryId="park.parkingManage.select_parkingInfo"/>
						</c:when>
						<c:otherwise>
					...
					<form:select id="kParkingCompCode" caption="관리회사" all="true" allLabel="전체" queryId="park.parkingManage.select_parking_comp_code"/>
