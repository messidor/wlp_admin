...
<script type="text/javascript">
	 ...
	 */
	 		...
	 		setColumn(["autoStatus", "입차 API 상태", "center", 100]),
	 		setColumn(["gpkiStatus", "감면API 상태", "center", 100]),
	 		setColumn(["outStatus", "출차 API 상태", "center", 100]),
	 		setColumn(["autoResultCode", "입차API 결과", "center", 100]),
	 		setColumn(["autoReceiveDt", "입차API 수신일시", "center", 125]),
	 		setColumn(["autoCarNo", "차량번호", "center", 100]),
	 		setColumn(["gpkiResultCode", "감면API 결과", "center", 100]),
	 		setColumn(["gpkiReceiveDt", "감면API 수신일시", "center", 125]),
	 		setColumn(["gpkiCarNo", "차량번호", "center", 100]),
	...
	setColumnMask(columnDefs, "autoReceiveDt", "datetime");
	setColumnMask(columnDefs, "gpkiReceiveDt", "datetime");
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "park.apiNewStatus.select_List");
		frm.addParam("dataType", "json"); // 그리드 불러오는 타입 (기본 json)
		frm.request();
	}

	function handleIQ(data, textStatus, jqXHR) {
		$("#autoReceiveDt, #autoTimeDiff, #gpkiReceiveDt, #gpkiTimeDiff, #outReceiveDt, #outTimeDiff").text("");
		$(".col-3 .card-block").height($(".col-9 .card-block").height()/3 - 67);
	}
	...
	function loadTableData2() {
		...
		frm.addParam("query_id", "park.apiNewStatus.select_lastReceive");
		frm.addParam("dataType", "json"); // 그리드 불러오는 타입 (기본 json)
		frm.request();
	}
	...
	function handleIQ2(data, textStatus, jqXHR) {
		if(data.length > 0){
			$("#autoReceiveDt").text(data[0].autoReceiveDt);
			$("#autoTimeDiff").text(data[0].autoTimeDiff);
			$("#gpkiReceiveDt").text(data[0].gpkiReceiveDt);
			$("#gpkiTimeDiff").text(data[0].gpkiTimeDiff);
...
<div class="page-body" style="font-size: 0.9em;">
	<div class="row">
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"
			...
			<div class="search-area col-lg-12">
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<form:select id="kParkingNo" caption="주차장 인증코드" all="true" allLabel="전체" queryId="park.apiNewStatus.select_parkInfo"/>
		...
		<div class="col-xl-3 col-lg-3 col-md-3 col-sm-3 col-3">
			...
			<card:content />
				<p>마지막 수신일시 : <span id="autoReceiveDt"></span></p>
				<p>미수신 시간&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : <span id="autoTimeDiff"></span></p>
			<card:close />
			
			<card:open title="입차(감면) API" />
			<card:toolButton title="">
			</card:toolButton>
			<card:content />
				<p>마지막 수신일시 : <span id="gpkiReceiveDt"></span></p>
				<p>미수신 시간&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; : <span id="gpkiTimeDiff"></span></p>
			<card:close />
