...
<script type="text/javascript">
	 ...
	 */
            ...
            setColumn(["parkingId", "주차장 ID", "center", 70]),
			setColumn(["gugunName", "구역(시/군/구)", "center", 70]),
            ...
            setColumn(["modName", "수정자", "center", 50]),
	...
	_gridHelper.addRowDblClickEvent(function(event) {
   		OpenPopupSingle("/park/generalParkingManagePopup.do?generalParkingNo=" + event.data.generalParkingNo, 1000, 850, "_GPP");
	});
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "park.generalParkingManage.select_list");
	...
	function loadComboData() {
		...
		frm.addParam("queryId", "park.generalParkingManage.select_parkingInfo");
	...
	$(function() {
		...
		$("#btnRegister").click(function() {
			OpenPopupSingle("/park/generalParkingManagePopup.do", 1000, 850, "_GPP");
		});
		
		// 엑셀 업로드 팝업
        $("#btnExcelUpload").click(function() {
            OpenPopup('<c:url value="/park/generalParkingUploadPopup.do" />', "_UploadPopup", 726, 1600, false, false, false);
        });
		...
		$("#kParkingNo,#kUseYn").change(function() {
			loadTableData();
		});
	...
	<div class="row">
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"
			...
			<card:button>
				...
				<form:button type="Upload" id="btnExcelUpload" className="bg_save" caption="엑셀업로드"/>
				...
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<c:choose>
						<c:when test="${fn:contains(user_role_id, 'MANAGE') || fn:contains(user_role_id, 'AD')}">
							...
							<form:select id="kParkingNo" caption="주차장" all="true" allLabel="전체" queryId="park.generalParkingManage.select_parkingInfo"/>
						</c:when>
						<c:otherwise>
