...
<script type="text/javascript">
	...
	function loadTableDetail() {
		...
		frm.addParam("query_id", "pay.elecReduction.Select_PopupList");
	...
	function handleIQ(data, textStatus, jqXHR) {
		if(data.length > 0){
			// 취소상태이면 승인/거절 버튼 숨김
			if(data[0].cancelGubnCode == "Y"){
				$("#rejectReason").parent().parent().parent().hide();
				$("#btnApproval, #btnRefusal").hide();
				
			}
			
			if(data[0].confirmGubnCode == "N"){
				$("#confirmName, #confirmDt").parent().parent().parent().hide();
				
			} else if(data[0].confirmGubnCode == "Y"){
				$("#rejectReason").parent().parent().parent().hide();
				$("#btnApproval, #btnRefusal").hide();
				
			} else if(data[0].confirmGubnCode == "C"){
				$("#confirmName, #confirmDt").parent().parent().parent().hide();
				$("#btnApproval, #btnRefusal").hide();
				
				$("#rejectReason").attr("readonly", true);
				$("#rejectReason").siblings("span").children("i").removeClass();
				$("#rejectReason").siblings("span").children("i").addClass("fas fa-eye");
				$("#rejectReason").css("background-color", "#EEEEEE");
			}
...
//      imageWin.document.title = "imagePopup";
    ...
    function validation(type){
		...
		if(type == "Refusal"){
			...
			text = "전기차 충전 할인 신청 건을 거절하시겠습니까?";
		}else{
			text = "전기차 충전 할인 신청 건을 승인하시겠습니까?";
		}
    ...
    function saveData(type) {
	    ...
	    frm.addParam("hMemberId", $("#hMemberId").val());
	    frm.addParam("rejectReason", $("#rejectReason").val());
	    frm.setAction("<c:url value='/payment/elecReductionUpdate.do' />");
		frm.request();
    }
		...
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
			<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
				...
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-horizontal" >
		            ...
		            <label:input id="confirmGubn" caption="승인구분" size="12" state="readonly" addAttr="style='text-align:center;'"/>
		            <label:input id="confirmName" caption="승인자" size="12" state="readonly"/>
		            <label:input id="confirmDt" caption="승인일시" size="12" state="readonly"/>
		            <label:textarea id="rejectReason" caption="거절사유" size="12" />
				</form>
				<card:close />
			</div>
			<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
