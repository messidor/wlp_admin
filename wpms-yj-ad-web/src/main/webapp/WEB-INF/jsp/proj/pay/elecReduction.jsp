...
<script type="text/javascript">
	...
	var columnDefs = [
		...
		*/
        ...
        setColumn(["confirmGubn", "승인구분", "center", 80]),
		setColumn(["cancelGubn", "취소구분", "center", 80]),
		setColumn(["confirmName", "승인자", "center", 80]),
...
// 		setColumn(["popDetail", "상세보기", "center", 80], [grdPopDetail, ["button", null, "link", "상세보기", ""]]),
	...
	function gridRowClickEvent(event) {
 		OpenPopupSingle("/pay/elecReductionPopup.do?elecApplyCode="+ event.data.elecApplyCode + "&memberId=" + event.data.memberId + "&carNo=" + event.data.carNo, 950, 660, "_PopElec");	
	} 
	
	function grdPopDetail(e, data) {
		OpenPopupSingle("/pay/elecReductionPopup.do?elecApplyCode="+ data.elecApplyCode + "&memberId=" + data.memberId + "&carNo=" + data.carNo, 950, 660, "_PopElec");	
	}
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "pay.elecReduction.Select_List");
			...
			<div class="search-area col-lg-12">
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						...
						<form:select id="kConfirmGubn" caption="승인구분" all="true" allLabel="전체" queryId="#CONFIRM_GUBN"/>
						<form:select id="kCancelGubn" caption="취소구분" all="true" allLabel="전체" queryId="#CANCEL_GUBN"/>
					</div>
				</form>
			</div>
			<card:close />
