...
<script type="text/javascript">
	...
	var columnDefs = [
		...
		*/
		...
		setColumn(["regDt", "처리일시", "center", 110]),
		...
		setColumn(["pcGubnName", "결제구분", "center", 80]),
		setColumn(["payPrice", "금액", "right", 80]),
	];
	
	// 그리드 하단
    var BottomcolumnDefs = [
    	setColumn(["no", "번호", "center", 50]),
    	setColumn(["regDt", "처리일시", "center", 110]),
		setColumn(["pGovName", "관리기관", "left", 150]),
		setColumn(["parkingName", "주차장명", "left", 130]),
		setColumn(["carNo", "차량번호", "center", 80]),
...
//     	    }],
	...
	function loadTableData() {
		...
		frm.addParam("query_id", "pay.dayPaymentManage.Select_List");
	    frm.addParam("dataType", "json");           // 그리드 불러오는 타입 (기본 json)
		frm.addParam("startDate", $("#kSearchDate").val());
	...
	function handleIQ(data, textStatus, jqXHR) {
		...
		$.each(data, function(k, v) {
			sum_price += parseInt(v.price);
	    });
		// 형식 변환
		sum_price = sum_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원";
		
		bottomRow[0].payPrice = sum_price;
		gridOptionsBottom.api.setRowData(bottomRow);
		$("#myGrid").height($("#myGrid").height()-32);
	}
	...
	function downExcelData() {
		...
		frm.addInput("startDate", $("#kSearchDate").val());
		frm.addInput("parkingGoveName", $("#kParkingGovCode option:checked").text());
		frm.addInput("query_id", "pay.dayPaymentManage.Select_List");
		frm.addInput("type", "dayPaymentManage");
        ...
        $("#frmMain > input[name='startDate']").remove();
	}
	...
	$(function() {
        ...
        var gridObj2 = new agGrid.Grid(gridDivBottom, gridOptionsBottom);
	  	...
	  	//일자
		$("#kSearchDate").initDateOnly("day", 0);
		...
		$("#kSearchDate, #kParkingNo, #kPcGubn").change(function() {
			loadTableData();
		});
		...
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"
			...
			<div class="search-area col-lg-12">
				<form id="frmMain" name="frmMain" method="post" onsubmit="return false" class="form-material form-inline">
					<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
						<form:input id="kSearchDate" caption="처리일자" addAttr="maxlength='100'" />
						<c:choose>  
						...
						<form:select id="kPcGubn" queryId="#PC_GUBN" caption="결제구분"  all="true" allLabel="전체" />
					</div>			
				</form>
			</div>
			<card:close />
