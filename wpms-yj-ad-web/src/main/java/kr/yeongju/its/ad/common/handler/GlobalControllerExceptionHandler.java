...
import kr.yeongju.its.ad.common.dto.ErrorInfo;
...
public class GlobalControllerExceptionHandler {
    ...
    private ResponseEntity<ErrorInfo> handleExceptionWithApiPath(HttpServletRequest req, Exception ex) {
        ...
        return ResponseEntity.status(status).contentType(MediaType.APPLICATION_JSON_UTF8).body((new ErrorInfo(req.getRequestURL().toString(), ex)));
    }
}
